import 'package:flutter/material.dart';
import 'package:font_awesome_flutter/font_awesome_flutter.dart';
import 'package:lend/core/models/list_details/list_details.dart';
import 'package:lend/presentation/pages/asset/widgets/details/detail_formatters.dart';
import 'package:lend/presentation/pages/asset/widgets/details/detail_item.widget.dart';
import 'package:lend/utilities/extensions/widget.extension.dart';

class GenericAssetDetails extends StatelessWidget {
  const GenericAssetDetails({super.key, required this.details});

  final GenericAssetListingDetails details;

  @override
  Widget build(BuildContext context) {
    final brandModel = [
      details.brand,
      details.model,
    ].map((item) => item.trim()).where((item) => item.isNotEmpty).join(' ');

    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        if (brandModel.isNotEmpty)
          AssetDetailItem(
            icon: FontAwesomeIcons.boxOpen,
            title: brandModel,
            subtitle: 'Asset details',
          ),
        if (nonEmptyText(details.notes) != null)
          AssetDetailNote(
            icon: FontAwesomeIcons.noteSticky,
            title: 'Notes',
            text: details.notes,
          ),
      ],
    ).withSpacing(16.0);
  }
}
